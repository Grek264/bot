...
async def processing_message(message: types.Message):
    ...
    if (user_id in user_id_base) == False:
        ...
        while boolean_variable == -1:
            boolean_variable = [index for index in range(len(user_base)) if user_id_base[index] == user_id]
            user_base.at[boolean_variable[0], 'name'] = message.text
            user_base.to_excel("user_base.xlsx")
        ...
        time_update(user_id=user_id)
        boolean_variable = [-1 if message.text == ADMIN_PASS else await bot.send_message(message.from_user.id, "Неверный пароль")]
        while boolean_variable == -1:
            boolean_variable = [index for index in range(len(user_base)) if user_id_base[index] == user_id]
            user_base.at[boolean_variable[0], 'admin'] = "YES", user_base.to_excel("user_base.xlsx")
        ...
        while boolean_variable == 1 and images != 11:
            ...
            if ('ничего не найдено' in response) == False:
                ...
                for image in Pictures:
                    ...
                    if ((f'{keyword} ' in chek) or (f'{keyword.lower()} ' in chek) or (keyword == chek) or (keyword.lower() == chek)) and images != 11:
                        ...
                        images += 1
                if maximum_number_of_pages[0] == num_page:
                    boolean_variable = 0
                else:
                    num_page += 1
            else:
        ...
        while boolean_variable == -1:
            boolean_variable = [index for index in range(len(user_base)) if user_id_base[index] == user_id]
            if status_base[boolean_variable[0]] == 'Online':
                await bot.send_message(message.from_user.id, f"Привет {user_name_base[boolean_variable[0]]}"), await bot.send_message(message.from_user.id, "/start\n/help\n/dice\n/func_menu")
            else:
                await bot.send_message(message.from_user.id,f"Привет {user_name_base[boolean_variable[0]]} давно не виделись"),await bot.send_message(message.from_user.id, "/start\n/help\n/dice\n/func_menu"), time_update(user_id=user_id)
